...
AUDIO_FILE_PATH = '/fs/nexus-projects/ego_data/adobe_music_generation/musicCaps/musicCaps_audio/'
...
def read_wav_file(filename, segment_length):
    ...
    # filename = '/sensei-fs/tenants/Sensei-AdobeResearchTeam/share-josephkj/sanjoy/audiocaps/audiocaps/data/deep/audioset/data/' + filename.split('/')[-1]
    
    try:
        # print("filename is ", filename)
        filename = AUDIO_FILE_PATH + filename.split('/')[-1]
        # print("trying to read filename: ", filename)
        waveform, sr = torchaudio.load(filename)  # Faster!!!
        waveform = torchaudio.functional.resample(waveform, orig_freq=sr, new_freq=16000)[0]
        
    except:
        # filename = '/sensei-fs/tenants/Sensei-AdobeResearchTeam/share-sanjoyc/' + 'CantinaBand3.wav'
        # waveform, sr = torchaudio.load(filename)
        print("can't read audio file")
