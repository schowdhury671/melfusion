...
class CrossAttnDownBlock2D(nn.Module):
    ...
    def forward(
        ...
        for resnet, attn in zip(self.resnets, self.attentions):
            ...
            # cross_attns_list_list_list.append(cross_attns_list_list)
        ...
        return hidden_states, output_states # , cross_attns_list_list_list
    ...
    def forward(
        ...
        for resnet, attn in zip(self.resnets, self.attentions):
            ...
            if self.training and self.gradient_checkpointing:
                ...
                hidden_states = torch.utils.checkpoint.checkpoint(create_custom_forward(resnet), hidden_states, temb)
                hidden_states, cross_attns_list_list = torch.utils.checkpoint.checkpoint(
                    create_custom_forward(attn, return_dict=False),
                    hidden_states,
                    encoder_hidden_states,
                    None,  # timestep
                ...
                hidden_states = resnet(hidden_states, temb)
                hidden_states, cross_attns_list_list = attn(
                    hidden_states,
                    encoder_hidden_states=encoder_hidden_states,
                    cross_attention_kwargs=cross_attention_kwargs,
                    attention_mask=attention_mask,
                    external_hidden_states=external_hidden_states[ite] if external_hidden_states is not None else None,
                    encoder_attention_mask=encoder_attention_mask,
                    return_dict=False
                )
                # hidden_states = hidden_states.sample
            ...
            # print('ite becomes: ', ite)

        if self.upsamplers is not None:
            for upsampler in self.upsamplers:
                hidden_states = upsampler(hidden_states, upsample_size)

        return hidden_states, cross_attns_list_list_list
