...
class Transformer2DModel(ModelMixin, ConfigMixin):
    ...
    def forward(
        ...
        for block in self.transformer_blocks:
            hidden_states, cross_attns = block(
                hidden_states,
                attention_mask=attention_mask,
                encoder_hidden_states=encoder_hidden_states,
                encoder_attention_mask=encoder_attention_mask,
                external_hidden_states = external_hidden_states[0] if external_hidden_states is not None else None,
                timestep=timestep,
                cross_attention_kwargs=cross_attention_kwargs,
                class_labels=class_labels,
            )
            cross_attns_list.append(cross_attns)
        # 3. Output
        if self.is_input_continuous:
            if not self.use_linear_projection:
        ...
        if not return_dict:
            return output,cross_attns_list # this is working
